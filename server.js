...
const { PORT, KEYMAP, PLAYERS } = require("./config");

keyboard.config.autoDelayMs = 0;

// ── Cloudflare Tunnel ──────────────────────────────────────────────────────
const CF_LOCAL = path.join(__dirname, "cloudflared.exe");
const CF_URL = "https://github.com/cloudflare/cloudflared/releases/latest/download/cloudflared-windows-amd64.exe";

let tunnelUrl = null;
let cfFound = null; // null=unknown, true=found, false=not found
...
function startTunnel(cfPath) {
  ...
  cf.on("error", (err) => {
    if (err.code === "ENOENT") { cfFound = false; console.log("[tunnel] cloudflared not found"); }
  });
  const onData = (data) => {
    const match = data.toString().match(/https:\/\/[a-z0-9-]+\.trycloudflare\.com/);
    if (match && !tunnelUrl) { tunnelUrl = match[0]; console.log(`[tunnel] ${tunnelUrl}`); }
  };
...
app.get("/api/tunnel-status", (_req, res) => res.json({
  tunnelUrl,
  cfFound,
  cfInstalled: fs.existsSync(CF_LOCAL),
  installing: install.running,
  installProgress: install.progress,
  installError: install.error,
}));

app.post("/api/uninstall-cloudflared", (req, res) => {
  try {
    if (fs.existsSync(CF_LOCAL)) fs.unlinkSync(CF_LOCAL);
    cfFound = false;
    tunnelUrl = null;
    install = { running: false, progress: 0, error: null };
    res.json({ ok: true });
  } catch (err) {
...
app.post("/api/install-cloudflared", async (req, res) => {
  if (install.running) return res.json({ ok: false, reason: "already running" });
  install = { running: true, progress: 0, error: null };
  res.json({ ok: true });
  try {
    await downloadFile(CF_URL, CF_LOCAL, (p) => { install.progress = p; });
    install.running = false;
    install.progress = 100;
    console.log("[tunnel] cloudflared installed, starting tunnel…");
    startTunnel(CF_LOCAL);
  } catch (err) {
    install.running = false;
    install.error = err.message;
    console.error("[tunnel] install error:", err.message);
  }
});
...
app.get("/api/qr", async (req, res) => {
  ...
  const svg = await QRCode.toString(target, {
    type: "svg", width: 420, margin: 2,
    color: { dark: "#120821", light: "#ffffff" },
  });
  res.type("svg").send(svg);
});

// ── Página do anfitrião (/host) ────────────────────────────────────────────
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .step-num {
      ...
      border-radius: 50%;
      background: linear-gradient(180deg, #6a2cc9, #4a1a99);
      display: flex;
      align-items: center;
      justify-content: center;
    ...
    .close-btn {
      ...
      border-radius: 50%;
      border: none;
      background: radial-gradient(circle at 50% 35%, #ff3b3b, #c1121f 65%, #8e0d18 100%);
      box-shadow: 0 5px 0 #6e0a12, 0 8px 18px rgba(0,0,0,0.5);
      color: #fff;
    ...
    .close-btn:active {
      ...
      box-shadow: 0 1px 0 #6e0a12, 0 3px 8px rgba(0,0,0,0.4);
    }
    .right {
      display: flex;
      flex-direction: column;
    ...
    .tab {
      flex: 1;
      padding: clamp(6px,1vh,10px) clamp(16px,2.5vw,32px);
      border: none;
      border-radius: 9px;
    ...
    .qr-wrap {
      ...
      line-height: 0;
      min-width: clamp(180px, 30vw, 380px);
      min-height: clamp(180px, 30vw, 380px);
      display: flex;
      align-items: center;
      justify-content: center;
...
<body>
  ...
  <script>
    const HOST_T = {
      en: {
        title: "How to join",
        s1: "Connect to the <strong>same WiFi</strong> as this PC.",
        s2: "Open your phone camera and scan the <strong>QR code →</strong>",
        s3: "Pick a free slot and enter your name.",
        s4: "Press the buttons and <strong>play!</strong>",
        tab_local: "Local",
        tab_remote: "Remote",
        tunnel_starting: "⏳ Starting tunnel…",
        not_found: "Cloudflared wasn't detected in your system, would you like to install? You can remove it later.",
        install_btn: "Install (≈35 MB)",
      ...
      pt: {
        title: "Como entrar",
        s1: "Liga-te à <strong>mesma rede WiFi</strong> que este PC.",
        s2: "Aponta a câmara do telemóvel ao <strong>código QR →</strong>",
        s3: "Escolhe o teu lugar e escreve o teu nome.",
        s4: "Carrega nos botões e <strong>joga!</strong>",
        tab_local: "Local",
        tab_remote: "Remoto",
        tunnel_starting: "⏳ A iniciar túnel…",
        not_found: "O Cloudflared não foi detetado no sistema.<p>Deseja instalar? Pode removê-lo mais tarde.",
        install_btn: "Instalar (≈35 MB)",
        installing: "A transferir cloudflared…",
    ...
    function showLoading() {
      inner.innerHTML = \`<span class="spin">⏳</span> \${tx("tunnel_starting")}\`;
    ...
    function showNotFound() {
      inner.innerHTML = \`
        <div style="text-align:center;padding:8px">
          <div style="font-size:clamp(0.78rem,1.25vw,1rem);opacity:0.85;margin-bottom:1.2em;line-height:1.5">\${tx("not_found")}</div>
          <button onclick="doInstall()" style="background:linear-gradient(180deg,#6a2cc9,#4a1a99);border:none;border-radius:10px;color:#fff;font-size:clamp(0.85rem,1.4vw,1rem);font-weight:700;padding:10px 20px;cursor:pointer">\${tx("install_btn")}</button>
        </div>\`;
      remoteUrl.textContent = "";
      document.getElementById("uninstall-btn").style.display = "none";
    }
    ...
    function showInstalling(pct) {
      inner.innerHTML = \`
        <div style="text-align:center;width:clamp(140px,22vw,300px)">
          <div style="font-size:clamp(0.8rem,1.3vw,1rem);opacity:0.9;margin-bottom:1em">\${tx("installing")}</div>
          <div style="background:rgba(255,255,255,0.12);border-radius:8px;height:10px;overflow:hidden">
            <div style="background:linear-gradient(90deg,#6a2cc9,#a855f7);height:100%;width:\${pct}%;border-radius:8px;transition:width 0.3s"></div>
          </div>
          <div style="font-size:clamp(0.75rem,1.1vw,0.9rem);opacity:0.6;margin-top:0.5em">\${pct}%</div>
        </div>\`;
      remoteUrl.textContent = "";
    }
    ...
    function showError(msg) {
      inner.innerHTML = \`
        <div style="text-align:center">
          <div style="font-size:clamp(0.8rem,1.3vw,1rem);color:#ff5555;margin-bottom:0.4em">\${tx("install_error")}</div>
          <div style="font-size:clamp(0.7rem,1.1vw,0.85rem);opacity:0.55;margin-bottom:1em">\${msg}</div>
          <button onclick="doInstall()" style="background:rgba(255,255,255,0.12);border:none;border-radius:10px;color:#fff;font-size:clamp(0.8rem,1.2vw,0.95rem);font-weight:700;padding:8px 18px;cursor:pointer">\${tx("retry_btn")}</button>
        </div>\`;
      remoteUrl.textContent = "";
    }
    ...
    async function doUninstall() {
      ...
      qrLoaded = false;
      document.getElementById("remote-qr-wrap").classList.remove("has-qr");
      pollRemote();
    }

    async function doInstall() {
      showInstalling(0);
      await fetch("/api/install-cloudflared", { method: "POST" });
    }
    ...
    async function pollRemote() {
      try {
        ...
        else if (s.cfFound === false) { knownNotFound = true; showNotFound(); }
        else { showLoading(); }
      } catch { showLoading(); }

      pollTimer = setTimeout(pollRemote, 1500);
    }
...
// Estado de slots em memoria: player(1-8) -> ws ocupante (ou null).
const slots = {};
const names = {};
...
function broadcastSlots() {
  const taken = {};
  for (let p = 1; p <= PLAYERS; p++) taken[p] = slots[p] !== null;
  const msg = JSON.stringify({ type: "slots", taken, names });
...
wss.on("connection", (ws) => {
  ws.player = null;
  const t = takenMap(); ws.send(JSON.stringify({ type: "slots", taken: t, names }));
  ...
  ws.on("message", async (raw) => {
    ...
    if (data.type === "join") {
      ...
      if (!(p >= 1 && p <= PLAYERS)) {
        ws.send(JSON.stringify({ type: "join_result", ok: false, reason: "slot invalido" }));
        return;
      }
      if (slots[p] && slots[p] !== ws) {
        ws.send(JSON.stringify({ type: "join_result", ok: false, reason: "ocupado" }));
        return;
      }
      if (ws.player && slots[ws.player] === ws) { slots[ws.player] = null; names[ws.player] = null; }
      slots[p] = ws;
      ws.player = p;
    ...
    if (data.type === "leave") {
      if (ws.player && slots[ws.player] === ws) {
        slots[ws.player] = null;
        names[ws.player] = null;
        ws.player = null;
        broadcastSlots();
      }
      return;
    ...
    if (data.type === "press") {
      if (!ws.player) return;
      await tapKey(ws.player, data.button);
      return;
    }
  });
...
function getLanIp() {
  ...
  for (const name of Object.keys(ifaces)) {
    for (const iface of ifaces[name]) {
      ...
      candidates.push({ addr: iface.address, name, isCgnat, isPrivateLan });
    }
  }
  const pick =
    candidates.find((c) => c.isPrivateLan && !c.isCgnat) ||
...
server.listen(PORT, () => {
  ...
  const url = `http://${ip}:${PORT}`;
  console.log("\n==================================================");
  console.log("  BuzzCast a correr!");
  console.log("==================================================");
  console.log(`  Anfitriao: http://localhost:${PORT}/host`);
  console.log(`  Telemoveis: ${url}`);
  console.log("==================================================\n");
  qrcode.generate(url, { small: true });
  // Abre a pagina do anfitriao automaticamente no browser
